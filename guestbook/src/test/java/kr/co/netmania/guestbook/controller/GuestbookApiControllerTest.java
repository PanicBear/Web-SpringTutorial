...
import kr.co.netmania.guestbook.config.WebMvcContextConfiguration;
...
public class GuestbookApiControllerTest {
	...
	public void getGuestbooks() throws Exception {
		...
		mockMvc.perform(reqBuilder).andExpect(jsonPath("$.list[0].name").value("kim")).andDo(print());
	...
	public void deleteGuestbook() throws Exception {
		...
		// Mock객체가 메서드를 호출했을 때
		// 특정 값을 리턴한다
		when(guestbookService.deleteGuestbook(id, ip)).thenReturn(1);
		
		// MockMvc에게 호출할 URL 생성
		// application/json 형식으로 /guestbooks를 get방식 호출
		RequestBuilder reqBuilder = MockMvcRequestBuilders.delete("/guestbooks/" + id)
				.contentType(MediaType.APPLICATION_JSON);
		
		// 생성된 URL을 통해 MockMvc에 요청을 보냄
		// 성공을 예측하기에 상태코드 200이 나와야함
		// 예측이 맞았을 경우, 처리내용 출력
		mockMvc.perform(reqBuilder).andExpect(status().isOk()).andDo(print());
		
		//verify(guestbookService).deleteGuestbook(id, ip);
	}
